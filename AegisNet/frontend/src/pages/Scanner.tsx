...
import { Target, Loader2, AlertCircle, Wifi, Play, Terminal, Activity, ShieldAlert, Monitor, Server, Smartphone, Router, HelpCircle, Layers } from 'lucide-react';
...
export default function Scanner() {
    ...
    const [activeTab, setActiveTab] = useState<'scan' | 'traffic'>('scan');
    ...
    const getOsIcon = (os: string) => {
        ...
        if (os.includes("Apple") || os.includes("macOS") || os.includes("iOS")) return <Smartphone className="text-slate-200" />;
            ...
            <header className="flex justify-between items-end">
                ...
                <div className="flex bg-white/5 p-1 rounded-lg border border-white/10">
                    <button
                        ...
                        className={`px-4 py-2 rounded-md font-bold text-sm transition-all flex items-center gap-2 ${activeTab === 'scan' ? 'bg-aegis-500 text-white shadow-lg' : 'text-slate-400 hover:text-white'}`}>
                        <Target size={16} /> Discovery
                    </button>
                    <button
                        onClick={() => setActiveTab('traffic')}
                        className={`px-4 py-2 rounded-md font-bold text-sm transition-all flex items-center gap-2 ${activeTab === 'traffic' ? 'bg-aegis-500 text-white shadow-lg' : 'text-slate-400 hover:text-white'}`}>
                        <Layers size={16} /> Traffic Analysis
                    </button>
                </div>
            </header>
                        ...
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
                            ...
                            <div className="grid grid-cols-1 gap-4">
                                {result.hosts.map(host => {
                                    const displayName = host.friendly_name || host.hostname || host.ip;
                                    const showIp = (host.friendly_name || host.hostname) && host.ip !== displayName;
                                                    ...
                                                    <div>
                                                        <div className="flex flex-col mb-1">
                                                            <div className="flex items-center gap-3">
                                                                ...
                                                                {showIp && (
                                                                    <span className="text-xs text-slate-400 bg-black/20 px-2 py-0.5 rounded">{host.ip}</span>
                                                                )}
                                                            </div>
                                                            {host.model && <div className="text-aegis-400 text-sm font-bold tracking-wide">{host.manufacturer} {host.model}</div>}
                                                        </div>

                                                        <div className="text-sm text-slate-400 font-mono mb-2">
                                                            {host.mac}
                                                            {host.vendor !== displayName && <span> • {host.vendor}</span>}
                                                            {!host.model && host.manufacturer && host.manufacturer !== host.vendor && host.manufacturer !== displayName && <span> • {host.manufacturer}</span>}
                                                        </div>
